...
def train_one_epoch(model, optimizer, data_loader, criterion, device, epoch,
    ...
    for i, (imgs, targets, paths, _, _, normal, mask) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # ni 统计从epoch0开始的所有batch数
        ni = i + nb * epoch  # number integrated batches (since train start)
        # pp = 0
        # imgs =imgs[:, pp, :, :, :].squeeze(1)
        ...
        if multi_scale:
            ...
            sf = img_size / max(imgs.shape[3:])  # scale factor

            # 如果图片最大边长不等于img_size, 则缩放图片，并将长和宽调整到32的整数倍
            if sf != 1:
                # gs: (pixels) grid size
                ns = [math.ceil(x * sf / gs) * gs for x in imgs.shape[3:]]  # new shape (stretched to 32-multiple)
                normal = F.interpolate(normal, size=ns, mode='bilinear', align_corners=False)
                mask = F.interpolate(mask, size=ns, mode='bilinear', align_corners=False)
                iimg=[]
                for i in range(imgs.shape[0]):
                    img = F.interpolate(imgs[i], size=ns, mode='bilinear', align_corners=False)
                    iimg.append(img)
                imgs = torch.stack(iimg, axis=0)
                

        # 混合精度训练上下文管理器，如果在CPU环境中不起任何作用
        
        with amp.autocast(enabled=scaler is not None):
            pred, normal_pred = model(imgs, mask)
            normal = normal.float()
            normal_pred = normal_pred.float()
            mask = mask.float()

            n_losses = criterion.forward(normal_pred, normal)
            # normal_pred = (normal_pred + 1) / 2
            # normal_pred = normal_pred * mask  
            # vutils.save_image(normal_pred, './normal_pred.jpg')  
            loss_dict = compute_loss(pred, targets, model.darknet)
            loss_dict["N_loss"]=torch.tensor(n_losses["N_loss"]).to(device)
            losses=(loss_dict["box_loss"]+loss_dict["obj_loss"] +loss_dict["class_loss"])+loss_dict["N_loss"].reshape(1)  
        ...
        loss_items = torch.cat((loss_dict_reduced["box_loss"],
                                    ...
                                    loss_dict_reduced["N_loss"].reshape(1),
...
def evaluate(model, data_loader, coco=None, device=None):
    ...
    for imgs, targets, paths, shapes, img_index,normal, mask in metric_logger.log_every(data_loader, 100, header):
        mask = mask.to(device).float() / 255.0
        imgs = imgs.to(device).float() / 255.0
        # pp = 0
        # imgg =imgs[:, pp, :, :, :].squeeze(1) # uint8 to float32, 0 - 255 to 0.0 - 1.0

        # normal = normal.to(device).float() / 255.0
        # targets = targets.to(device)
        # 当使用CPU时，跳过GPU相关指令
        ...
        pred = model(imgs, mask)[0]  # only get inference result
...
class Criterion(object):
    ...
    def setupNormalCrit(self, args):
        ...
        if args.cuda:
            self.n_crit = self.n_crit.to(args.device)

    def forward(self, output, target):#[1, 3, 960, 960]
        if self.normal_loss == 'cos':
            ...
            self.out_reshape = output.permute(0, 2, 3, 1).contiguous().view(-1, 3)#*(mask.permute(0, 2, 3, 1).contiguous().view(-1, 3))
