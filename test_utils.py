...
import numpy as np
from models_co_attention import model_utils
...
def test(args, split, loader, model, log, epoch, recorder):
    ...
    with torch.no_grad():
        for i, sample in enumerate(loader):
            ...
            out_var = model(data['input']); timer.updateTime('Forward')
            ...
            if iters % save_intv == 0:
                ...
                log.saveNormalResults(masked_pred, split, epoch, iters, sample['name'])
